...
MOUNTFLAGS := -v $(CURDIR)/data:/home/alpine -v $(CURDIR)/data/host:/etc/ssh
...
PORTFLAGS  := -p 64822:64822
PROXYFLAGS := --build-arg http_proxy=$(http_proxy) --build-arg https_proxy=$(https_proxy) --build-arg no_proxy=$(no_proxy)

RUNFLAGS   := -c 128 -m 256m -e PGID=$(PUID) -e PUID=$(PUID) # -e CNTUSER=alpine -e CNTPASS=notsosecure -e ROOTPASS=notsosecureroot#-e PGID=$(PGID) #  gid 100 usually pre exists
...
test :
	docker run --rm -it $(NAMEFLAGS) $(RUNFLAGS) $(PORTFLAGS) $(MOUNTFLAGS) $(OTHERFLAGS) --entrypoint sh $(IMAGETAG) -ec 'git --version; openssl version; rsync --version;ssh -V;'
